...
const createPatient = asyncHandlar(async (req, res) => {
    try {
        const {name, diagonalwidth, Address, age, bloodgroup, gender,mobileNumber, password, admittedIn  } = req.body
    
        
         if ( [name , diagonalwidth , Address , age , bloodgroup , gender , mobileNumber , password, admittedIn  ].some((field) => typeof field === 'string' && field.trim() === ""))
            {
...
const getPatientDetails = asyncHandlar(async(req, res) => {
    ...
    try {
        ...
        return res
        .status(200)
        .json(new ApiResponce(200, patient, "Patient details fetched successfully."))
    } catch (error) {
        return res
        .status(error.statusCode || 500)
        .json({message: error.message || "Something went wrong"})
...
const updatePatient = asyncHandlar(async(req, res) => {
    ...
    try {
        ...
        return res
        .status(200)
        .json(new ApiResponce(200, patient, "Patient details updated successfully."))
    } catch (error) {
        return res
        .status(error.statusCode || 500)
        .json({message: error.message || "Something went wrong"})
...
const addMedicalRecord = asyncHandlar(async(req, res) => {
    ...
    try {
       ...
       patient.medicalRecords.push(newRecord._id) 
        ...
        return res
        .status(200)
        .json(new ApiResponce(200, newRecord, "Medical record added successfully."))
...
const getAllMedicalRecords = asyncHandlar(async(req, res) => {
    ...
    try {
        ...
        const medicalRecord = await MedicalRecord.findById(recordId).populate("patientId" , "name");
        ...
        return res
        .status(200)
        .json(new ApiResponce(200, medicalRecord ,"Medical records fetched successfully." ))
...
const patientMedicalBill = asyncHandlar(async(req, res) => {
    ...
    try {
        const { treatmentDetails, totalAmount, paymentStatus = 'Unpaid'  } = req.body;
...
const getMedicalBill = asyncHandlar(async(req, res) => {
    ...
    try {
        // Find medical Bill
        const bill = await MedicalBill.findById(billId).populate("patientId" , "name")
